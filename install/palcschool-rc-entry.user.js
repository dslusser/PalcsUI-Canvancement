...
(function () {
    ...
    var PalcschoolRCE = function() {
        ...
        // The values below hold the list of MARKING PERIOD OR PROGRESS CHECK comments
        // to be added for grades A, B, C, D, and F.
        ...
        // The values below hold the list of COURSE FINAL GRADE comments to be added for grades A, B, C, D, and F.
        // Refer to the list above to add or remove specific comments to the following list.
        // These comments lists should be the only thing that needs to be modified.
        var finalCommentsForGradeA = [comment1, comment2, comment4]; // CSV of comments for students earning > than 90%
        var finalCommentsForGradeB = [comment1, comment2]; // CSV of comments for students earning >= 80% and < 90%
        var finalCommentsForGradeC = [comment6]; // CSV of comments for students earning >= 70% and < 80%
        var finalCommentsForGradeD = [comment28]; // CSV of comments for students earning >= 60% and < 70%
        var finalCommentsForGradeF = [comment32, comment33];// CSV of comments for students earning < than 60%
        ...
        function copyGradesAndComments() {
            ...
            // Loop for final course comments
            for (var iFinal = 0; iFinal < studentIDsArray.length; iFinal++) {
                var stuidFinal = studentIDsArray[iFinal];

                var stu1Final = document.getElementById('finalgrade_' + stuidFinal);
                var stu1valFinal = parseInt(stu1Final.value);

                if (isNaN(stu1valFinal)) {
                    console.log(stu1valFinal + ' is not a number.');
                } else if (stu1valFinal >= 90) {
                    for (var aFinal = 0; aFinal < commentsForGradeA.length; aFinal++) {
                        document.getElementById('finalcomments_' + stuidFinal + '_' + finalCommentsForGradeA[aFinal]).checked = true;
                    }
                } else if (stu1valFinal >= 80 && stu1valFinal < 90) {
                    for (var bFinal = 0; bFinal < commentsForGradeB.length; bFinal++) {
                        document.getElementById('finalcomments_' + stuidFinal + '_' + finalCommentsForGradeB[bFinal]).checked = true;
                    }
                } else if (stu1valFinal >= 70 && stu1valFinal < 80) {
                    for (var cFinal = 0; cFinal < commentsForGradeC.length; cFinal++) {
                        document.getElementById('finalcomments_' + stuidFinal + '_' + finalCommentsForGradeC[cFinal]).checked = true;
                    }
                } else if (stu1valFinal >= 60 && stu1valFinal < 70) {
                    for (var dFinal = 0; dFinal < commentsForGradeD.length; dFinal++) {
                        document.getElementById('finalcomments_' + stuidFinal + '_' + finalCommentsForGradeC[dFinal]).checked = true;
                    }
                } else if (stu1valFinal < 60) {
                    for (var fFinal = 0; fFinal < commentsForGradeF.length; fFinal++) {
                        document.getElementById('finalcomments_' + stuidFinal + '_' + finalCommentsForGradeF[fFinal]).checked = true;
                    }
                }
            }
        }
        ...
        function addGradesAndCommentsLink() {
            ...
            var infomsgTextContent = infomsg.innerText;

             if (infomsgTextContent.includes("If student grades looks out of date, please click Refresh below")) {
            ...
            } else if (infomsgTextContent.includes("Grading not open for course.")) {

                var gradingInfoMsg = document.getElementsByClassName('infomsg')[0].innerText;
                var infomsgtext = 'Grading not open for course.';

                console.log(gradingInfoMsg)

                if (gradingInfoMsg == infomsgtext && infomsgtext != 'undefined'){

                console.log(gradingInfoMsg)
                console.log("Grading might not be open for this course")
